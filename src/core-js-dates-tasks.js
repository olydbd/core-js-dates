...
function getTime(date) {
  return String(new Date(date)).split(' ')[4];
}
...
function getNextFridayThe13th(date) {
  ...
  dateCopy.setDate(dateCopy.getDate() + 5 - dateCopy.getDay());

  if (dateCopy.getDate() === 13) return dateCopy;

  return getNextFridayThe13th(
    new Date(dateCopy.setDate(dateCopy.getDate() + 7))
  );
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let countWorkDaysCopy = countWorkDays;

  const res = [];

  while (startDate <= endDate) {
    res.push(refactorDate(startDate));
    countWorkDaysCopy -= 1;
    if (!countWorkDaysCopy) {
      startDate.setDate(startDate.getDate() + countOffDays + 1);
      countWorkDaysCopy = countWorkDays;
    } else {
